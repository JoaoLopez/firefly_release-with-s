...
_FIREFLY_DIR = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
sys.path.append(os.path.join(_FIREFLY_DIR, "python"))
os.environ["FF_DIR"] = _FIREFLY_DIR
os.environ["STELLARPOPMODELS_DIR"] = os.path.join(_FIREFLY_DIR, "stellar_population_models")
...
input_file=os.path.join(_FIREFLY_DIR, 'example_data', 'spec-0266-51602-0001.dat')
data = np.loadtxt(input_file, unpack=True)
suffix = ""

#redshift
redshift = 0.100000000
...
@deterministic
def run_firefly(redshift, wavelength, flux, error, func_globals=None):
	print('')
...
def main():
	t0=time.time()
	run_firefly(redshift, wavelength, flux, error, func_globals=globals())
	print()
